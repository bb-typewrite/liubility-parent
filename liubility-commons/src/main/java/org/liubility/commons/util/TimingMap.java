...
public class TimingMap<K, V> extends HashMap<K, V> {
    // 存放过期日期的map
    private final Map<Object, Long> timingMap = new HashMap<>();
    // 过期时间
    ...
    public V put(K key, V value) {
        timingMap.put(key, System.currentTimeMillis() + expireTime);
    ...
    public TimingMap() {
        ...
        TimerTask task = new TimerTask() { // 创建一个新的计时器任务。
            ...
            public void run() {
                ...
                for (Object key : timingMap.keySet()) {
     ...
     * @param keys
     */
    public void removeExpireData(List<Object> keys) {
        for (Object key : keys) {
...
//            return null;
//        }
        timingMap.put(key, currentTime + expireTime);
